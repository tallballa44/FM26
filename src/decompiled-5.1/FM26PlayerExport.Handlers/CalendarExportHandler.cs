...
public class CalendarExportHandler : IExportHandler
{
	...
	public void FinishCapture()
	{
		...
		stringBuilder.AppendLine("</head><body><h2>Resumo: Calendário</h2><br><table><tr>");
		...
		if (flag)
		{
			((BepInExLogInterpolatedStringHandler)val).AppendLiteral("[FM26Export.Calendar] Exportou ");
			((BepInExLogInterpolatedStringHandler)val).AppendFormatted<int>(_rows.Count);
			((BepInExLogInterpolatedStringHandler)val).AppendLiteral(" linhas de calendário para ");
