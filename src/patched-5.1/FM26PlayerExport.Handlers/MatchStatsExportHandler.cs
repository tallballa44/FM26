...
public class MatchStatsExportHandler : IExportHandler
{
	...
	public bool TryStartCapture(VisualElement root, out string errorMessage)
	{
		...
		{
			errorMessage = "[FM26Export.MatchStats] Match tabs (KeyStatistics, Passing...) were not found in the match UI.";
			return false;
		}
	...
	public bool CaptureStep()
	{
		...
		if (val != null && val2 != null)
		{
			...
			if (flag)
			{
				...
				((BepInExLogInterpolatedStringHandler)val5).AppendLiteral(" rows from ");
	...
	public void FinishCapture()
	{
		...
		try
		{
			...
			string value3 = (string.IsNullOrEmpty(_matchContext) ? "Unified Statistics" : ("Resumo: " + _matchContext));
			...
			if (flag2)
			{
				...
				((BepInExLogInterpolatedStringHandler)val).AppendLiteral("/6) tabs read, ");
				((BepInExLogInterpolatedStringHandler)val).AppendFormatted<int>(num);
				((BepInExLogInterpolatedStringHandler)val).AppendLiteral(" rows exported.");
			}
			log.LogInfo(val);
			ManualLogSource log2 = Plugin.Log;
